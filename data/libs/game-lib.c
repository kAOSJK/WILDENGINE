...
void print_credits(int y_max, int x_max, char *story_writer)
{
    WINDOW *drawin;
    WINDOW *drawin_2;
    WINDOW *win;
    ...
    int x_pos = x_max / 2 - (width / 2);
    bool found_drawings = false;

    /* CREDITS WINDOW CREATION */
    win = create_newwin(height, width, y_pos, x_pos);
    if (strcmp(story_writer, "") != 0)
        mvwprintw(win, 3, 3, "Story writter: %s", story_writer);
    else
        mvwprintw(win, 3, 3, "Unknow story writter");
    mvwprintw(win, 6, 3, "Engine Programmer: Covarel Edgar");
    wrefresh(win);
    ...
    while (1)
    {
        ...
        move_res = reload_credits_win(win, y_pos, x_pos, story_writer);
        if (move_res == -1)
...
int reload_credits_win(WINDOW *win, int y_pos, int x_pos, char *story_writer)
{
    int move_res;
