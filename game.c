...
void play_game(int, int, char *, char **, char *, char *, char **);
void play_agility_game(int, int, int, char *, char *, char *);
char *ask_new_name(int, int, size_t, char *, char *);

int main()
{
    /* BASICS VARs */
    char *story_writer = NULL;
    char *language = NULL;
    char *buffer = NULL;
    int y_max, x_max = 0;
    ...
    story_writer = get_json_object_string("story_writer", buffer);
    ...
    /* PLAY THE MENU */
    play_menu(y_max, x_max, &language, story_writer, buffer);
...
void play_menu(int y_max, int x_max, char **language_ptr, char *story_writer, char *buffer)
{
    /* BASIC VARs */
    int menu_answer;
    int agility_speed;
    ...
    menu_answer = display_menu(y_max, x_max, language_ptr, buffer);
    ...
    if (menu_answer == 0)
    {
        ...
        play_game(y_max, x_max, buffer, &usr_buffer, name, story_writer, language_ptr);
    }
    else if (menu_answer == 1)
    {
        ...
        play_agility_game(y_max, x_max, agility_speed, language, story_writer, buffer);
    }
    else if (menu_answer == 2)
    {
        ...
        *language_ptr = display_languages(y_max, x_max, buffer);
        ...
        play_menu(y_max, x_max, language_ptr, story_writer, buffer);
    }
    else if (menu_answer == 3)
    {
        print_credits(y_max, x_max, story_writer);
        play_menu(y_max, x_max, language_ptr, story_writer, buffer);
    }
...
void play_game(int y_max, int x_max, char *buffer, char **usr_buffer_ptr, char *name, char *story_writer, char **language_ptr)
{
    /* STORY JSON VARs */
    ...
    for (i = 0; story[i]; i++)
    {
        ...
        /* BEGIN THE CHAPTER */
        begin_chapter(y_max, x_max, speed_0, speed_1, agility_speed, language_ptr, story[i], i, parsed_story, name, buffer, usr_buffer_ptr);
...
void play_agility_game(int y_max, int x_max, int speed, char *language, char *story_writer, char *buffer)
{
    /* BASICS VARs */
    WINDOW *win;
    int height = y_max / 4;
